...
- **多语言标签**：**已在源头修复（P0-6，2026-07-19）**。此前约 554 个节点名称、834 个
  节点描述的中文与英文相同（占位符），另有大量中英混杂标签；现全部 2436 个节点名称与描述
  均已重译为 genuine 中文（zh==en 占位符 = 0，无中文残留 = 0；英文源不变；KPI 名称同步
  重译）。附录 A 的多语言标签机制由此**实质满足**，而非以剔除占位符的方式规避。导出脚本
  仍保留 zh==en 过滤器作为防御性守卫。
